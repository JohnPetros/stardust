...
import { SignInWithGoogleAccountController } from '../SignInWithGoogleAccountController'

describe('Sign In With Google Account Controller', () => {
  let http: Mock<Http>
  ...
  beforeEach(() => {
    ...
    controller = new SignInWithGoogleAccountController(service)
  })
  ...
  it('should call the auth service with the correct return url and redirect to the sign in url', async () => {
    ...
    service.signInWithGoogleAccount.mockResolvedValue(restResponse)

    await controller.handle(http)

    expect(service.signInWithGoogleAccount).toHaveBeenCalledWith(returnUrl)
