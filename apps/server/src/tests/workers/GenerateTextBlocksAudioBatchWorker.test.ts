...
import { GenerateTextBlocksAudioBatchJob } from '@/queue/jobs/lesson'
import { InngestFixture } from '../fixtures/InngestFixture'
import { TEXT_BLOCKS_AUDIO_GENERATION_IN_BATCH_REQUESTED_EVENT_NAME } from '@/queue/inngest/constants/lesson-event-names'
...
jest.mock('@/queue/jobs/lesson', () => {
  ...
  const generateJob = Object.assign(
    jest.fn().mockImplementation(() => ({
      handle: mockJobHandle,
    })),
    { KEY: actual.GenerateTextBlocksAudioBatchJob.KEY },
  )

  return {
    ...actual,
    GenerateTextBlocksAudioBatchJob: generateJob,
  }
})

describe('GenerateTextBlocksAudioBatchWorker', () => {
  ...
  beforeAll(async () => {
    ...
    await inngest.setup(app, [GenerateTextBlocksAudioBatchJob.KEY])
  })
  ...
  beforeEach(() => {
    ...
    jest.mocked(GenerateTextBlocksAudioBatchJob).mockClear()
  })

  it('executes the batch audio job from an Inngest event', async () => {
    const response = await inngest.send({
      id: randomUUID(),
      name: TEXT_BLOCKS_AUDIO_GENERATION_IN_BATCH_REQUESTED_EVENT_NAME,
      data: {
    ...
    expect(GenerateTextBlocksAudioBatchJob).toHaveBeenCalledWith(expect.anything())
