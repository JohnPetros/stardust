...
import { CancelTextBlockAudioGenerationJob } from '@/queue/jobs/lesson'
import { TEXT_BLOCK_AUDIO_GENERATION_CANCELLED_EVENT_NAME } from '@/queue/inngest/constants/lesson-event-names'
...
jest.mock('@/queue/jobs/lesson', () => {
  ...
  const cancelJob = Object.assign(
    jest.fn().mockImplementation(() => ({
      handle: mockJobHandle,
    })),
    { KEY: actual.CancelTextBlockAudioGenerationJob.KEY },
  )

  return {
    ...actual,
    CancelTextBlockAudioGenerationJob: cancelJob,
  }
})

describe('CancelTextBlockAudioGenerationWorker', () => {
  ...
  beforeAll(async () => {
    ...
    await inngest.setup(app, [CancelTextBlockAudioGenerationJob.KEY])
  })
  ...
  beforeEach(() => {
    ...
    jest.mocked(CancelTextBlockAudioGenerationJob).mockClear()
  })

  it('executes the cancellation job from an Inngest event', async () => {
    const response = await inngest.send({
      id: randomUUID(),
      name: TEXT_BLOCK_AUDIO_GENERATION_CANCELLED_EVENT_NAME,
      data: {
    ...
    expect(CancelTextBlockAudioGenerationJob).toHaveBeenCalledWith(expect.anything())
