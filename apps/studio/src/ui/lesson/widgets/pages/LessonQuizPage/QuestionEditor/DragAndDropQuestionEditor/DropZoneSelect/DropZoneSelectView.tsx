...
export const DropZoneSelectView = ({
  ...
  return (
    <Select
      value={isItemEmpty ? 'não selecionado' : selectedItem}
    ...
    >
      ...
      <SelectContent className='bg-zinc-800'>
        {items.map((item, index) => (
          <SelectItem
            key={item}
            value={item ? item : index.toString()}
            disabled={correctItems.includes(item)}
            className='text-zinc-100 hover:bg-zinc-600'
          >
