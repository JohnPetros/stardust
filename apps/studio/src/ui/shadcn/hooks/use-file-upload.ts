...
export type FileUploadOptions = {
  ...
  onFilesAdded?: (addedFiles: File[]) => void // Callback when new files are added
}
  ...
  const validateFile = useCallback(
    (file: File | FileMetadata): string | null => {
      ...
      if (accept !== '*') {
        ...
        if (!isAccepted) {
          return `Arquivo "${file instanceof File ? file.name : file.name}" não é um arquivo aceito.`
        }
      }
  ...
  const addFiles = useCallback(
    (newFiles: FileList | File[]) => {
      ...
      if (
        ...
        errors.push(`Você pode apenas enviar um máximo de ${maxFiles} arquivos.`)
        setState((prev) => ({ ...prev, errors }))
        return
      }
      ...
      if (validFiles.length > 0) {
        ...
        onFilesAdded?.(validFiles.map((file) => file.file as File))

        setState((prev) => {
          const newFiles = !multiple ? validFiles : [...prev.files, ...validFiles]
          onFilesChange?.(newFiles.map((file) => file.file as File))
          return {
            ...prev,
            files: newFiles,
            errors,
