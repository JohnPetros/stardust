...
const TOOLBAR_BUTTON_CLASS_NAME =
  'h-6 w-max items-center rounded cursor-pointer bg-green-400 px-4 text-xs font-medium text-gray-900 transition-[scale] duration-200 active:scale-95'
...
export const CodeSnippetView = ({
  ...
  return (
    <div
      className={twMerge(
        'not-prose relative w-full overflow-hidden rounded-md bg-zinc-800',
        isRunnable ? `h-[${editorHeight}px]` : 'h-auto',
      )}
    >
      <ToolBar.Root className='not-prose flex items-center justify-end gap-2 border-b border-zinc-700 p-2'>
        {isRunnable && (
          <>
