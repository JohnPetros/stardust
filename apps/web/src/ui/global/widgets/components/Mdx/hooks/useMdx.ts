...
export function useMdx() {
  ...
  const parseTextBlocksToMdx = useCallback(
    (textBlocks: TextBlock[], audioFiles?: Record<string, boolean>) => {
      function getPropValue(prop: string, textBlockDto: TextBlockDto) {
        return textBlockDto[prop as keyof TextBlockDto]
      }

      function parseOtherProps(textBlockDto: TextBlockDto, canIncludeAudio: boolean) {
        ...
        const parsedDefaultProps = otherProps
          .map(
            (prop) =>
              `${prop}={'${
                prop === 'title'
        ...
        if (!canIncludeAudio) return parsedDefaultProps

        const audioFileName = textBlockDto.audio?.fileName?.trim()
        const audioStatus = textBlockDto.audio?.status

        if (!audioFileName || !audioStatus || audioFiles?.[audioFileName] !== true) {
          return parsedDefaultProps
        }
      ...
      function parseMdx(textBlock: TextBlock) {
        const canIncludeAudio = textBlock.canHaveAudio.value
        const props = parseOtherProps(textBlock.dto, canIncludeAudio)
