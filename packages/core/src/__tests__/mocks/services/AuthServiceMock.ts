...
export class AuthServiceMock implements IAuthService {
  ...
  signUp(email: string, password: string, name: string): Promise<ApiResponse<{ userId: string }>> {
    throw new Error('Method not implemented.')
  }
  resetPassword(newPassword: string, accessToken: string, refreshToken: string): Promise<ApiResponse> {
    throw new Error('Method not implemented.')
  }
