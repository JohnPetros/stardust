import type { Baseline, Metrics, Violation } from './define-metrics'
...
export function buildReport(
  ...
  lines.push(`## Quality Ratchet — ${baseline.workspace}`)
  lines.push('')
  lines.push(
    passed
      ? '✅ **Passou** — nenhuma métrica piorou.'
      : `❌ **Falhou** — ${violations.length} regressão(ões).`,
  )
  lines.push('')
  ...
  lines.push(
    row(
      'Biome warnings',
      base.biomeWarnings,
      current.biomeWarnings,
      current.biomeWarnings <= base.biomeWarnings,
    ),
  )
  lines.push(
    row(
      'Escape hatches de tipo',
      base.typeEscapes.total,
      current.typeEscapes.total,
      current.typeEscapes.total <= base.typeEscapes.total,
    ),
  )
  lines.push(
    row(
  ...
  for (const [layer, baseCoverage] of Object.entries(base.coverage)) {
    ...
    lines.push(
      row(
        ...
        currentCoverage
          ? `${currentCoverage.lines}% / ${currentCoverage.branches}%`
          : 'ausente',
        ok,
      ),
    )
  ...
  if (!passed) {
    ...
    for (const violation of violations) {
      lines.push(
        `| ${violation.metric} | ${violation.detail} | ${violation.baseline} | ${violation.current} |`,
      )
    }
    lines.push('')
    lines.push(
      '> A catraca só anda num sentido: corrija as regressões acima ou, se for melhoria intencional do baseline, rode com `--update-baseline`.',
    )
  }
