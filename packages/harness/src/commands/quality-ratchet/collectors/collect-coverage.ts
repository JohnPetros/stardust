...
import type { LayerCoverage, Metrics } from '../define-metrics'
...
export function collectCoverage(workspace: WorkspaceConfig): Metrics['coverage'] {
  ...
  if (!resolveLayer) {
    throw new Error(
      `Workspace "${workspace.name}" mede cobertura mas não define resolveLayer.`,
    )
  }
  const workspaceRoot = fromRepoRoot(workspace.dir)
  const coverageSourceGlobs = workspace.coverageSourceGlobs ?? [
    `${workspace.srcDir}/**/*.ts`,
  ]
