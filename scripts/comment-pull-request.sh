...
#
# Uso: comment-pull-request.sh <workspace> <report-file>
# Requer no ambiente: GH_TOKEN, REPO (owner/repo), PR_NUMBER.

WORKSPACE="${1:?uso: comment-pull-request.sh <workspace> <report-file>}"
REPORT_FILE="${2:?uso: comment-pull-request.sh <workspace> <report-file>}"
...
MARKER="<!-- quality-ratchet:${WORKSPACE} -->"
LEGACY_MARKER="<!-- quality-gate:${WORKSPACE} -->"
...
COMMENT_ID="$(gh api "repos/${REPO}/issues/${PR_NUMBER}/comments" --paginate \
  --jq "[.[] | select(.body | contains(\"${MARKER}\") or contains(\"${LEGACY_MARKER}\"))][0].id // empty")"
