...
→ judge-implementation-agent
→ judge-conclusion-agent
→ fechamento, commit e PR
```
...
`[x]` significa exclusivamente que a tarefa foi aceita pelo
`judge-implementation-agent`.
...
→ judge-implementation-agent avalia
├── failed: findings retornam ao builder-agent
└── accepted: orchestrator-agent marca [x]
```
...
- O Implementation Gate combina escopo, `codecheck`, `typecheck`, `test:unit`,
  `quality-ratchet`, arquitetura, migrations e Contract.
...
- Revisão humana: reabre tarefa como `changes_requested` e registra `HR-*`.

### 7. Conclusão — `conclude-spec` + `judge-conclusion-agent`

`conclude-spec` só começa depois da aceitação de todas as tarefas ou da
...
4. Aciona `judge-conclusion-agent` com diff integral e contexto limpo.
5. Se falhar, reabre tarefas; não corrige código dentro da conclusão.
6. Se aceitar, fecha Spec e Plan, marca PRD e prepara commit e PR.
